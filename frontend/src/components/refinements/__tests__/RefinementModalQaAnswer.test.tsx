...
import type { RefinementQAItem } from '@/types';

const apiMock = vi.hoisted(() => ({
  listRefinementQA: vi.fn(),
  createRefinementQuestion: vi.fn(),
  createRefinementChoiceQuestion: vi.fn(),
...
import { QATab } from '../RefinementModal';

const choiceQuestion: RefinementQAItem = {
  id: 'qa-1',
  refinement_id: 'ref-1',
...
const textQuestion: RefinementQAItem = {
  ...choiceQuestion,
  id: 'qa-2',
...
function renderQATab(items: RefinementQAItem[] = [choiceQuestion]) {
  apiMock.listRefinementQA.mockResolvedValue(items);
  return render(<QATab refinementId="ref-1" mentionables={[]} onChanged={vi.fn()} />);
}
...
describe('RefinementModal Q&A answer submission', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('disables the submit button while the answer is in flight and re-enables it after', async () => {
    const pending = deferred<RefinementQAItem>();
    apiMock.answerRefinementQuestion.mockReturnValue(pending.promise);
    renderQATab();
  ...
  it('calls the API exactly once when the submit button is double clicked', async () => {
    ...
    await waitFor(() => expect(apiMock.answerRefinementQuestion).toHaveBeenCalledTimes(1));
  ...
  it('reloads the Q&A list and retries once on a retryable 409 conflict', async () => {
    apiMock.answerRefinementQuestion
      .mockRejectedValueOnce(conflict())
      .mockResolvedValueOnce({ ...choiceQuestion, answered_at: '2026-09-02T00:00:00Z' });
    renderQATab();

    const submit = await openChoiceAnswerForm();
    await waitFor(() => expect(apiMock.listRefinementQA).toHaveBeenCalledTimes(1));
    fireEvent.click(submit);

    await waitFor(() => expect(toastMock.success).toHaveBeenCalledWith('Answer posted'));
    expect(apiMock.answerRefinementQuestion).toHaveBeenCalledTimes(2);
    // one refresh before the retry + the usual post-success refresh
    expect(apiMock.listRefinementQA).toHaveBeenCalledTimes(3);
  ...
  it('does not retry a non-retryable error and shows the server message', async () => {
    apiMock.answerRefinementQuestion.mockRejectedValue(
