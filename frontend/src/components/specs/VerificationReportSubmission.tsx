...
function ReportForm({ specId, scenario, allowedResults, canSubmit, onSaved, onRejected }: Props) {
  ...
  if (!canSubmit || !['inspection', 'static_analysis', 'demonstration'].includes(scenario.verification_method || '')
    || !allowedResults.some(result => result === 'passed' || result === 'failed')) return null;
  async function submit() {
    ...
    try {
      ...
      if (!report || Array.isArray(report) || report.method !== scenario.verification_method
        || (report.result !== 'passed' && report.result !== 'failed') || !allowedResults.includes(report.result)) {
      ...
      await api.updateTestScenarioStatus(specId, scenario.id, { status: result as 'passed' | 'failed', evidence: admitted.evidence });
      if (!live.current) return;
      setSaved(true);
  ...
  return <div className="space-y-2 rounded border p-3">
    <p className="text-xs">Record an external {scenario.verification_method?.replace(/_/g, ' ')} report. Include versioned sources, observations for every linked criterion and the result. Submission does not approve the Test Card.</p>
