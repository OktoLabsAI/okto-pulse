...
from legacy_sprint_schema import Base as HistoricalBase, Card as LegacyCard, Sprint
...
async def test_card_policy_read_preserves_values_scope_and_source_data(tmp_path, monkeypatch, source):
    ...
    legacy = source in {"live", "missing", "foreign"}
    sprint_id = "missing" if source == "missing" else "sprint"
    metadata = HistoricalBase.metadata if legacy else Base.metadata
    writes = []
    statements = []
    commits = []
    ...
    try:
        ...
        async with sessions() as db:
            ...
            if legacy:
                ...
                db.add(LegacyCard(id="card", board_id="board", spec_id="spec", title="Task", created_by="owner",
                                  sprint_id=sprint_id))
            else:
        ...
        if legacy:
            # The removed mapper cannot see an unmigrated policy origin. Startup
            # must refuse this source before any operational read can default it.
            ...
            async with sessions() as db:
                assert (await db.get(LegacyCard, "card")).sprint_id == sprint_id
