...
async def test_cleanup_preserves_authority_and_retains_exact_before_after_without_credentials(database):
    ...
    assert len(RETIRED) == 53  # 20 maintenance operations and 33 Sprint leaves.
    assert receipt.changed_documents == 5 and receipt.removed_entries == 57 and receipt.review_markers == 6
