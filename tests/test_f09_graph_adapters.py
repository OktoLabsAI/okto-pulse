...
def test_f09_local_storage_locator_round_trips_only_inside_adapter(
    tmp_path: Path,
) -> None:
...
def test_f09_global_runtime_exposes_typed_contract_without_native_openers() -> None:
    ...
    runtime = CommunityGrafxGlobalDiscoveryRuntime(
        lambda: None, lambda: Path("unused"), lambda: None, lambda _phase: None
    )
    assert isinstance(runtime, GlobalDiscoveryRuntime)
    public_methods = {
        name
        for name, _ in inspect.getmembers(runtime, predicate=callable)
        if not name.startswith("_")
    }
    assert "open_connection" not in public_methods
    assert "open_kuzu_db" not in public_methods
    assert (
        inspect.signature(runtime.execute).return_annotation == "GraphStatementResult"
    )


def test_f09_community_owns_graph_ddl_pool_and_concrete_search_adapter() -> None:
    community_sources = {
        "graph_ddl": COMMUNITY_ROOT
        / "src"
        / "okto_pulse"
        / "community"
        / "adapters"
        ...
        / "community"
        / "adapters"
        / "grafx_database_pool.py",
        "hybrid": COMMUNITY_ROOT
        / "src"
        / "okto_pulse"
        / "community"
        / "adapters"
        / "hybrid_search.py",
        "chaos": COMMUNITY_ROOT
        / "src"
        / "okto_pulse"
        / "community"
        / "adapters"
        / "kg_chaos_executor.py",
    }
    ...
    assert not (
        CORE_ROOT / "src" / "okto_pulse" / "core" / "kg" / "connection_pool.py"
    ).exists()
    assert not (
        CORE_ROOT
        / "src"
