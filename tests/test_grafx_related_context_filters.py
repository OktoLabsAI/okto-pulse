...
def graph(request):
    ...
    for name in (
        ...
        attrs = _attrs(
            name,
            SOURCE if name in {"orphan", "center"} else f"filter:{name}",
            "filter-test",
        )
    ...
    with database.begin("write") as tx:
        tx.execute(
            "CREATE (:Entity {id:'filter:ct',title:'trace',source_artifact_ref:'filter:ct-source',"
        ...
        tx.execute(
            "MATCH (a:Decision {id:'filter:ct-center'}),(b:Entity {id:'filter:ct'}) "
        ...
        for source, target, kind in (
            ...
            tx.execute(
                f"MATCH (a:Decision {{id:$source}}),(b:Decision {{id:$target}}) "
...
def read(graph, **kwargs):
    return graph[0].find_by_artifact_filtered(
        BOARD_ID,
        SOURCE,
        QueryFilters(min_confidence=0.5, max_rows=50),
...
def test_result_limit_does_not_cut_off_earlier_isolated_centers(graph):
    rows = graph[0].find_by_artifact_filtered(
        BOARD_ID,
        SOURCE,
        QueryFilters(min_confidence=0.5, max_rows=1),
        direction="outgoing",
        max_depth=1,
        graph_layer="canonical",
    )
...
def test_default_filtered_shape_matches_unfiltered(graph):
    ...
    expected = graph[0].find_by_artifact(
        BOARD_ID, SOURCE, filters, graph_layer="canonical"
    )
...
def test_superseded_opt_in_and_layer_are_preserved(graph):
    rows = graph[0].find_by_artifact_filtered(
        BOARD_ID,
        SOURCE,
        QueryFilters(min_confidence=0.5, max_rows=50, include_superseded=True),
        direction="outgoing",
        max_depth=1,
        graph_layer="all",
    )
...
def test_depth_one_never_expands_a_second_hop(graph, monkeypatch):
    ...
    assert all(
        options == {"direction": "outgoing", "rel_types": frozenset({"supersedes"})}
...
def test_code_traceability_is_filtered_at_center_and_neighbour(graph, source):
    ...
    assert (
        graph[0].find_by_artifact_filtered(
            BOARD_ID,
            source,
            filters,
            max_depth=1,
...
def test_core_service_uses_filtered_capability_through_routed_facade(
    graph, monkeypatch
):
    ...
    monkeypatch.setattr(
        service, "_cached_call", lambda _name, _board, _params, call: call()
    )
    rows = service.get_related_context(
        BOARD_ID,
        SOURCE,
        rel_types=["supersedes"],
        direction="outgoing",
        max_depth=1,
    )
    assert [row["hop1_id"] for row in rows] == ["filter:out", "filter:out"]
    assert all(
        row["hop2_id"] is None and row["rel1_type"] == "supersedes" for row in rows
    )
...
def test_routed_facade_preserves_operation_window_and_all_filter_arguments():
    ...
    assert facade.find_by_artifact_filtered(BOARD_ID, SOURCE, filters, **kwargs) == [
        ["kept"]
    ]
    assert events == [
        ("enter", BOARD_ID),
        ((BOARD_ID, SOURCE, filters), kwargs),
        ("exit", BOARD_ID),
    ]
