...
@router.get("/boards/{board_id}/historical-context/{target_kind}/{target_id}")
async def read_historical_context(
    response: Response,
    board_id: str = Path(min_length=1, max_length=255),
    target_kind: Literal["spec", "card"] = Path(),
    ...
    try:
        ...
        raise HTTPException(422, detail="Historical context request invalid", headers={"Cache-Control": "no-store"}) from exc
    try:
        ...
        raise HTTPException(413, detail="Historical context reading limit exceeded", headers={"Cache-Control": "no-store"}) from exc
    except (ArchiveReadUnavailable, ConsistentReadContractError) as exc:
        raise HTTPException(503, detail="Historical context unavailable", headers={"Cache-Control": "no-store"}) from exc
