...
HELP = {
    ...
    "buffer_budget_bytes": "Maximum page-cache capacity per handle, not total process RAM. Pulse normally has one writer and two readers per board: multiply by three and by resident boards. Buffers fill on demand.",
    ...
    "codec": "Page codec: pure or numpy. NumPy requires the acceleration dependency; supported codecs preserve identical persisted page bytes. This is independent from vector math.",
    "vector_math": "Vector arithmetic: auto currently selects the deterministic pure implementation; numpy explicitly uses the accelerator. NumPy requires the dependency and may change floating-point tie/ranking details within documented tolerances.",
