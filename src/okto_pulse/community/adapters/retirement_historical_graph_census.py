...
def read_graph_record_census(reader, *, deadline, budget):
    ...
    for batch in reader.iter_nodes(batch_size=500):
        ...
        for node in batch:
            ...
            budget[0] += len(_encode(record))
            if len(nodes) >= _MAX_NODES or budget[0] > _LIMIT:
                raise ValueError('retirement_historical_census_limit')
            nodes.append(record)
    for batch in reader.iter_relations(batch_size=500):
        ...
        for relation in batch:
            ...
            if key not in relations:
                budget[0] += len(_encode(key)) + 32
            if budget[0] > _LIMIT:
                raise ValueError('retirement_historical_census_limit')
            relations[key] += 1
    if measured.counts() != counts:
        raise ValueError('retirement_historical_census_counts_changed')
    return measured, {
        'schema_digest': measured.schema_hex, 'fingerprint': measured.digest(),
        'counts': asdict(measured.counts()),
...
def read_retirement_historical_graph_census(snapshot, *, max_seconds=180):
    ...
    for graph in manifest['graphs']:
        ...
        graphs.append({'scope': graph['scope'], 'board_id': graph['board_id'],
            'source_sha256': graph['sha256'], **records})
...
def compare_graph_record_censuses(before_graphs, after_graphs, *, deadline):
    """Delegate exact comparison to Core; no semantic classification implied."""
    old = {(row['scope'], row['board_id']): row for row in before_graphs}
    new = {(row['scope'], row['board_id']): row for row in after_graphs}
